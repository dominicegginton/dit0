...
impl Server for HttpsServer {
    fn spawn(self, handle: tokio::runtime::Handle) -> anyhow::Result<()> {
        std::thread::spawn(move || {
            let listener = match self.ts_net.listen("tcp", ":443") {
                Ok(l) => l,
                Err(e) => {
...
impl HttpServer {
    pub fn new(ts_net: Arc<libtailscale::Tailscale>) -> Self {
        let app = Router::new().layer(TraceLayer::new_for_http());

        HttpServer { app, ts_net }
    }
}
...
impl Server for HttpServer {
    fn spawn(self, handle: tokio::runtime::Handle) -> anyhow::Result<()> {
        std::thread::spawn(move || {
            if let Err(e) = self.ts_net.enable_funnel_to_localhost_plaintext_http1(3389) {
                tracing::error!("Failed to enable funnel to localhost: {}", e);
                return;
            }

            let bind_addr = "127.0.0.1:3389";
            let listener = match std::net::TcpListener::bind(&bind_addr) {
                Ok(l) => l,
                Err(e) => {
                    tracing::error!("Failed to bind to localhost {}: {}", bind_addr, e);
                    return;
                }
            };
                ...
                match listener.accept() {
                    Ok((stream, _sock_addr)) => {
                        ...
                        handle.spawn(async move {
                            ...
                            let service = hyper::service::service_fn(
                                move |_: Request<hyper::body::Incoming>| {
                                    async move {
                                        let html = String::from("<html><body><h1>TSDIT</h1><p>This page was served over the tailscale funnel service so that SSL Certificates are generated by Tailscale and trusted by clients. Please restart the service, and reload the page.</p></body></html>");
